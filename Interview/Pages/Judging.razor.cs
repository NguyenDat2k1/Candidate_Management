...
namespace Interview.Pages
{
    public partial class Judging
    {
        ...
        string /*text,*/  statusContact, Place, Body, mailND, locationPV, tenUV, Reason;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            userDetails = await client.GetFromJsonAsync<List<Users>>("/api/getAllUserNeedToJudge");
        ...
        private async Task<List<Users>> Searching()
        {
            ...
            var client = httpClient.CreateClient();
            client.BaseAddress = new Uri("https://localhost:7142");
            userSearchs = await client.GetFromJsonAsync<List<Users>>($"/api/searchUserNeedToJudge?research={transText}");
        ...
        private async Task<List<Users>> Filtering()
        {
            ...
            else
            {
                ...
                var client = httpClient.CreateClient();
                client.BaseAddress = new Uri("https://localhost:7142");
                userFilters = await client.GetFromJsonAsync<List<Users>>($"/api/filterUserNeedToJudge?filterBy={transText}");
                userDetails = await JudgeService.getUserNeedToJudge();
            }
            return userFilters;
        ...
        private async Task<List<Users>> Ordering()
        {
            ...
            else
            {
                var client = httpClient.CreateClient();
                client.BaseAddress = new Uri("https://localhost:7142");
                userOrders = await client.GetFromJsonAsync<List<Users>>($"/api/orderUserNeedToJudge?order={order}");
                userDetails = await JudgeService.getUserNeedToJudge();
                //return userOrders;
            }
        ...
        /// <summary>
        /// Phương thức thực hiện mở popup để đánh giá.
        /// </summary>
        ///<param name="user"
        /// <returns>.</returns>
        ...
        /// <summary>
        /// Phương thức thực hiện lưu đánh giá .
        /// </summary>
        ///
        /// <returns>.</returns>
        private void handleJudge()
        {
            ...
            if (lookFlg)
            {
                foreach (var uDetails in looking)
                {
                    ...
                    {
                        uDetails.Applied = Users.Applied;
                        uDetails.userID = Users.userID;
                        uDetails.Comment = Users.Comment;                      
                         Tempo.Add(uDetails);
                        if (isChecked1 == false && isChecked2 == true)
                        {
                            uDetails.Status = 19;
                           JudgeService.UploadResultFailed(uDetails.Applied, uDetails.userID, uDetails.Comment, uDetails.Status);
                        }
                        else if (isChecked2 == false && isChecked1 == true)
                        {
                            if (uDetails.Position == "Intern"&& uDetails.typeMail==1)
                            {
                                uDetails.Status = 15;
                            }
                            else
                            ...
                            JudgeService.UploadResultPassed(uDetails.userID, uDetails.Comment, uDetails.Status);
                        }
                    }
        ...
        private async Task<List<Users>> Looking()
        {
            ...
            if (search != "" && type == "" && Order == "")
            {
                ...
                else
                {
                    ...
                    var client = httpClient.CreateClient();
                    client.BaseAddress = new Uri("https://localhost:7142");
                    looking = await client.GetFromJsonAsync<List<Users>>($"/api/searchUserNeedToJudge?research={transText}");
                }
            ...
            else if (search == "" && type != "" && Order == "")
            {
                ...
                var client = httpClient.CreateClient();
                client.BaseAddress = new Uri("https://localhost:7142");
                looking = await client.GetFromJsonAsync<List<Users>>($"/api/filterUserNeedToJudge?filterBy={transText}");
            ...
            else if (search == "" && type == "" && Order != "")
            {
                ...
                var client = httpClient.CreateClient();
                client.BaseAddress = new Uri("https://localhost:7142");
                looking = await client.GetFromJsonAsync<List<Users>>($"/api/orderUserNeedToJudge?order={order}");
